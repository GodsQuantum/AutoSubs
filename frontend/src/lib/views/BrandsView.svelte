...
<div class="page">
  ...
  <div class="split-editor">
    ...
    <div class="stack">
      <section class="card"><div class="card-header"><strong>{draft.name||$dictionary.newBrand}</strong>{#if draft.id}<button class="btn danger" on:click={remove}>{$dictionary.delete}</button>{/if}</div><div class="card-body stack"><div class="field"><label>{$dictionary.brandName}</label><input class="input" bind:value={draft.name}/></div><div class="field"><label>{$dictionary.description}</label><textarea class="textarea" bind:value={draft.description}></textarea></div></div></section>
      <div class="grid two">
        <section class="card"><div class="card-header"><strong>{$dictionary.assets}</strong><div class="row"><button class="btn" on:click={()=>assetInput?.click()}>{$dictionary.uploadAsset}</button><button class="btn" on:click={()=>importPicker=true}>{$dictionary.importAsset}</button></div></div><div class="card-body stack">
          ...
          <div class="field"><label>{$dictionary.logo}</label><select class="select" bind:value={draft.assets.logo}><option value="">{$dictionary.none}</option>{#each assets.filter(a=>a.mime.startsWith('image/')) as a}<option value={a.id}>{a.name}</option>{/each}</select></div>
          ...
          <div class="field"><label>{$dictionary.defaultOutro}</label><select class="select" bind:value={draft.assets.defaultOutro}><option value="">{$dictionary.noOutro}</option>{#each assets as a}<option value={a.id}>{a.name}</option>{/each}</select></div>
