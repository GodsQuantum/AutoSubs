<script lang="ts">
  ...
  function selectFont(event:Event){const font=fonts.find(candidate=>candidate.id===(event.currentTarget as HTMLSelectElement).value);if(font)draft={...draft,fontFamily:font.fullName||font.family,bold:font.weight>=600,italic:font.italic};}
  ...
  async function checkSelectedFont(family:string,font:FontFace|null,_stylesheetRevision:number){
    ...
    if(!font){fontStatus=null;return;}
    ...
    <div class="stack">
      ...
      <div class="grid two">
        <section class="card"><div class="card-header"><strong>{$dictionary.styling}</strong></div><div class="card-body stack">
          <div class="grid two"><div class="field"><label for="presets-field-7">{$dictionary.animation}</label><select id="presets-field-7" class="select" bind:value={draft.animationStyle}><option value="pop">{$dictionary.pop}</option><option value="highlight">{$dictionary.highlight}</option><option value="karaoke">{$dictionary.karaoke}</option><option value="word-by-word">{$dictionary.wordByWord}</option><option value="fade">{$dictionary.fade}</option><option value="slide-up">{$dictionary.slideUp}</option><option value="bounce">{$dictionary.bounce}</option><option value="none">{$dictionary.animationNone}</option></select></div><div class="field"><label for="presets-field-8">{$dictionary.font}</label><select id="presets-field-8" class="select" value={selectedCustomFont?.id ?? ""} on:change={selectFont}>{#if !selectedCustomFont}<option value="">{draft.fontFamily}</option>{/if}{#each fonts as font}<option value={font.id}>{font.fileName} · {font.fullName || font.family} · {font.style}</option>{/each}</select>{#if selectedCustomFont && fontStatus}<span class="help" class:font-loaded={fontStatus.status === "loaded"}>{fontStatus.status === "loaded" ? "✓ Custom font loaded" : "⚠ Custom font fallback"} · {fontStatus.fileName}</span>{/if}</div></div>
      ...
      <section class="card">
        ...
        <div class="card-body"><div class="preview-shell"><FormatPreview format={draft.format} preset={draft} {fonts} text={$dictionary.sampleText} {safeZone} editable={true} onPositionChange={(x,y)=>{draft.positionX=Math.round(x);draft.positionY=Math.round(y);draft={...draft};}}/></div></div>
      </section>
    </div>
  </div>
</div>
