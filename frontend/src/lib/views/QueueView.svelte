<script lang="ts">
  ...
  async function action(job:Job, kind:'prepare'|'render'|'cancel'|'retranscribe'|'delete') {
    if(busyJobs.has(job.id) || (kind==='delete' && !confirm($dictionary.confirmDelete)))return;
    ...
    finally{const next=new Set(busyJobs);next.delete(job.id);busyJobs=next;}
  }
  const active = (job:Job) => ['pending','uploading','probing','transcribing','correcting','rendering'].includes(job.status);
  const outputFileName=(job:Job)=>job.outputPath?.split(/[\\/]/).pop()||`autosubs-${job.id}.mp4`;
</script>
      ...
      {#each jobs as job (job.id)}
        <article class="job-row">
          ...
          <div class="job-actions">
            ...
            {#if job.status==='error'||job.status==='cancelled'||job.status==='interrupted'}<button class="btn primary" disabled={busyJobs.has(job.id)} on:click={()=>action(job,'prepare')}>↻ {$dictionary.retry}</button>{/if}
            {#if active(job)}<button class="btn danger" disabled={busyJobs.has(job.id)} on:click={()=>action(job,'cancel')}>■ {$dictionary.cancel}</button>{/if}
            {#if !active(job)}<button class="btn danger" disabled={busyJobs.has(job.id)} on:click={()=>action(job,'delete')}>× {$dictionary.delete}</button>{/if}
          </div>
