  ...
  <div class="split-editor">
    ...
    <div class="stack">
      ...
      <section class="card"><div class="card-header"><strong>{$dictionary.outputSection}</strong></div><div class="card-body grid two">
        ...
        <div class="field"><label for="workflows-field-6">{$dictionary.presetOverride}</label><select id="workflows-field-6" class="select" bind:value={draft.presetId}><option value="">{$dictionary.brandDefault}</option>{#each presets as p}<option value={p.id}>{p.name}</option>{/each}</select></div>
        ...
        <div class="field"><label for="workflows-field-8">{$dictionary.fit}</label><select id="workflows-field-8" class="select" bind:value={draft.format.fit} disabled={draft.format.key==='source'}><option value="preserve">{$dictionary.preserve}</option><option value="contain">{$dictionary.contain}</option><option value="cover">{$dictionary.cover}</option><option value="stretch">{$dictionary.stretch}</option></select></div>
        {#if draft.format.key==='custom'}<div class="field"><label for="workflows-field-9">{$dictionary.width}</label><input id="workflows-field-9" class="input" type="number" bind:value={draft.format.width}/></div><div class="field"><label for="workflows-field-10">{$dictionary.height}</label><input id="workflows-field-10" class="input" type="number" bind:value={draft.format.height}/></div>{/if}
      </div></section>
