<script lang="ts">
  ...
  import type { Asset, Brand, Capabilities, FontFace, Job, Preset, SettingsView, Workflow } from '$lib/types';
  ...
  let jobs:Job[]=[];let presets:Preset[]=[];let brands:Brand[]=[];let workflows:Workflow[]=[];let assets:Asset[]=[];let fonts:FontFace[]=[];let settings:SettingsView|undefined;let capabilities:Capabilities|undefined;
  ...
  async function loadAll(){
    try{
      const [j,p,b,w,a,f,s,c]=await Promise.all([api.jobs(),api.presets(),api.brands(),api.workflows(),api.assets(),api.fonts(),api.settings(),api.capabilities()]);
      jobs=j;presets=p;brands=b;workflows=w;assets=a;fonts=f;settings=s;capabilities=c;initialError='';
  ...
  onMount(()=>{
    const fontStylesheet=document.createElement('link');fontStylesheet.rel='stylesheet';fontStylesheet.href='/api/v1/fonts/css';document.head.appendChild(fontStylesheet);
