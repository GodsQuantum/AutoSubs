...
pub async fn list_assets(State(state): State<AppState>) -> AppResult<Json<Vec<Asset>>> {
    Ok(Json(state.db.list("asset").map_err(AppError::Internal)?))
}
...
pub async fn upload_asset(
        ...
        .map_err(|e| AppError::BadRequest(e.to_string()))?
    {
        ...
        let name = field.file_name().unwrap_or("asset.bin").to_owned();
            ...
            .map_err(|e| AppError::BadRequest(e.to_string()))?
        {
            ...
            if size > state.config.max_upload_bytes {
                ...
                return Err(AppError::BadRequest(
                    "asset exceeds AUTOSUBS_MAX_UPLOAD_BYTES".into(),
                ));
            }
