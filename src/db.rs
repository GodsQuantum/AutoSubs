...
impl Database {
    ...
    pub fn list<T: DeserializeOwned>(&self, kind: &str) -> Result<Vec<T>> {
        let conn = self.conn();
        let mut stmt =
            conn.prepare("SELECT json FROM objects WHERE kind=?1 ORDER BY updated_at DESC,id")?;
    ...
    pub fn get_singleton<T: DeserializeOwned>(&self, kind: &str) -> Result<Option<T>> {
        let json: Option<String> = self
            .conn()
            .query_row("SELECT json FROM singleton WHERE kind=?1", [kind], |row| {
                row.get(0)
            })
            .optional()?;
        json.map(|v| serde_json::from_str(&v).map_err(Into::into))
