...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| "autosubs=info,tower_http=info".into()))
    ...
    if !caps.ffmpeg { tracing::warn!("FFmpeg is unavailable; the UI/API will run but media jobs will fail until FFmpeg is installed"); }
    else if !caps.libass { tracing::warn!("FFmpeg is available without the ass/libass filter; subtitle burn is disabled"); }
    *state.encoders.write().await = caps;
    ...
    if state.config.dist_dir.is_dir() {
        ...
        app = app.fallback_service(ServeDir::new(&state.config.dist_dir).append_index_html_on_directories(true).not_found_service(ServeFile::new(index)));
    }
