use crate::{
    config::Config,
    db::Database,
    domain::{Brand, FormatKey, Job, JobEvent, JobStatus, Preset, Settings, Workflow},
...
impl AppState {
    pub async fn load(mut config: Config) -> Result<Self> {
        ...
        if db.get_singleton::<Settings>("settings")?.is_none() {
            db.set_singleton("settings", &settings)?;
        }
        ...
        if presets.is_empty() {
            ...
            for preset in &presets {
                db.upsert("preset", &preset.id, preset)?;
            }
        } else {
        ...
        for workflow in &mut workflows {
            if workflow.migrate(&presets) {
                db.upsert("workflow", &workflow.id, workflow)?;
            }
        }
        ...
        let http = reqwest::Client::builder()
            ...
            .user_agent(concat!("AutoSubs/", env!("CARGO_PKG_VERSION")))
            .build()?;
        let render = config.max_render_jobs.max(1);
        let transcribe = config.max_transcription_jobs.max(1);
        let active = config.max_queued_jobs.max(1);
        Ok(Self {
...
fn default_presets() -> Vec<Preset> {
    ...
    clean.migrate();
    let mut karaoke = Preset {
        name: "Karaoke".into(),
        animation_style: crate::domain::AnimationStyle::Karaoke,
        size: 30.0,
        max_chars: 24,
        max_lines: 2,
        ..Preset::default()
...
fn migrate_legacy_brands(
    db: &Database,
    presets: &mut [Preset],
    brands: &mut Vec<Brand>,
) -> Result<()> {
    let names: Vec<String> = presets
        .iter()
        .filter_map(|p| p.legacy_brand.clone())
        .collect();
    for name in names {
        let brand_id =
            if let Some(brand) = brands.iter().find(|b| b.name.eq_ignore_ascii_case(&name)) {
                brand.id.clone()
            } else {
        ...
        for preset in presets
            .iter_mut()
            .filter(|p| p.legacy_brand.as_deref() == Some(name.as_str()))
        {
    ...
    for brand in brands.iter_mut() {
        ...
        for key in FormatKey::ALL {
            if brand.default_preset_by_format.contains_key(&key) {
                continue;
            }
            if let Some(preset) = presets
                .iter()
                .find(|p| p.brand_id.as_deref() == Some(brand.id.as_str()) && p.format.key == key)
            {
                brand
                    .default_preset_by_format
                    .insert(key, preset.id.clone());
            }
        }
        db.upsert("brand", &brand.id, brand)?;
    }
