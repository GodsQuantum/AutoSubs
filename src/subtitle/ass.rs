...
pub fn generate_ass_content(
    ...
    let (play_x, play_y) = preset
        ...
        .unwrap_or((1920, 1080));
    let requested_x =
        ((preset.position_x.clamp(0.0, 100.0) / 100.0) * play_x as f64).round() as i32;
    let requested_y =
        ((preset.position_y.clamp(0.0, 100.0) / 100.0) * play_y as f64).round() as i32;
...
"#,
    ...
    for line in normalized {
        ...
        let min_y = (play_y as f64 * 0.05 + block_height / 2.0).round() as i32;
        let max_y = (play_y as f64 * 0.95 - block_height / 2.0).round() as i32;
        let x = requested_x.clamp(min_x, max_x);
        let y = requested_y.clamp(min_y, max_y);
                ...
                for active in 0..total_words {
                    ...
                    let start = if active == 0 {
                        ...
                        words
                            .get(active)
                            .map(|w| w.start)
                            .unwrap_or(line.start + active as f64 * duration)
                    };
                    let end = words
                        .get(active + 1)
                        .map(|w| w.start)
...
mod tests {
    ...
    #[test]
    fn explicit_positions_keep_the_complete_subtitle_inside_graphics_safe_margins() {
        let preset = Preset {
            position_x: 0.0,
            position_y: 100.0,
            size: 60.0,
        ...
        let ass = generate_ass_content(&[sample_line()], &preset, Some((1920, 1080)));
        assert!(!ass.contains("\\pos(0,1080)"), "{ass}");
        assert!(!ass.contains("\\pos(0,0)"), "{ass}");
    }
    ...
    fn word_animations_use_word_duration_and_pop_returns_to_base_scale() {
        ...
        assert!(ass.contains("Dialogue: 0,0:00:01.00,0:00:01.60"));
        assert!(ass.contains("Dialogue: 0,0:00:01.60,0:00:02.00"));
