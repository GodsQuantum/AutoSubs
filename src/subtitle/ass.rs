...
pub fn to_ass_color(hex: &str, alpha_percent: u8) -> String {
    ...
    fn byte(value: &str) -> Option<u8> { u8::from_str_radix(value, 16).ok() }
    match value.len() {
        6 => {
            let (Some(r), Some(g), Some(b)) = (byte(&value[0..2]), byte(&value[2..4]), byte(&value[4..6])) else { return "&H00FFFFFF&".into(); };
            let alpha = ((alpha_percent.min(100) as f64 / 100.0) * 255.0).round() as u8;
            format!("&H{alpha:02X}{b:02X}{g:02X}{r:02X}&")
        }
        8 => {
            let (Some(r), Some(g), Some(b), Some(a)) = (byte(&value[0..2]), byte(&value[2..4]), byte(&value[4..6]), byte(&value[6..8])) else { return "&H00FFFFFF&".into(); };
...
fn safe_text(value: &str, uppercase: bool) -> String {
    ...
    if uppercase { escaped.to_uppercase() } else { escaped }
}

fn float_tags(duration_ms: i64, speed: f64) -> String {
    if duration_ms <= 0 || speed <= 0.0 { return String::new(); }
    ...
    while start < duration_ms {
        ...
        tags.push_str(&format!("{{\\t({start},{one},\\frz-1.5)\\t({one},{two},\\frz1.5)\\t({two},{end},\\frz0)}}"));
        start += cycle;
    }
    tags
}

pub fn generate_ass_content(lines: &[SubtitleLine], preset: &Preset, source_resolution: Option<(u32, u32)>) -> String {
    let normalized = normalize_subtitles(lines, NormalizeOptions::default()).lines;
    let (play_x, play_y) = preset.format.resolution(source_resolution).unwrap_or((1920, 1080));
    ...
    let mut out = format!(r#"[Script Info]
ScriptType: v4.00+
PlayResX: {play_x}
PlayResY: {play_y}
WrapStyle: 2
...
"#,
    ...
    for line in normalized {
        let visual_lines: Vec<&str> = line.text.split('\n').filter(|v| !v.trim().is_empty()).collect();
        if visual_lines.is_empty() { continue; }
        let total_words = visual_lines.iter().map(|v| v.split_whitespace().count()).sum::<usize>();
        if total_words == 0 { continue; }
        let words = line.words.clone().unwrap_or_default();
        let total_height = visual_lines.len().saturating_sub(1) as f64 * (preset.size + preset.line_spacing);
        ...
        match preset.animation_style {
            AnimationStyle::Pop => {
                for active in 0..total_words {
                    let start = if active == 0 { line.start } else { words.get(active).map(|w| w.start).unwrap_or(line.start) };
                    let end = if active + 1 >= total_words { line.end } else { words.get(active + 1).map(|w| w.start).unwrap_or(line.end).max(start + 0.02) };
                    let float = if preset.floating { float_tags(((end - start) * 1000.0) as i64, preset.wobble_speed) } else { String::new() };
                    let mut global_index = 0usize;
                    for (visual_index, visual) in visual_lines.iter().enumerate() {
                        let curr_y = (start_y + visual_index as f64 * (preset.size + preset.line_spacing)).round() as i32;
                        let mut rendered = format!("{{\\pos({x},{curr_y})}}{float}");
                        for (word_index, token) in visual.split_whitespace().enumerate() {
                            if word_index > 0 { rendered.push(' '); }
                            ...
                            } else { rendered.push_str(&token); }
                            global_index += 1;
                        }
                        out.push_str(&format!("Dialogue: 0,{},{},Default,{name},0,0,0,,{}\n", format_ass_time(start), format_ass_time(end), rendered));
                    }
                }
            }
            _ => {
                let duration_ms = ((line.end - line.start) * 1000.0).round() as i64;
                let float = if preset.floating { float_tags(duration_ms, preset.wobble_speed) } else { String::new() };
                ...
                for (visual_index, visual) in visual_lines.iter().enumerate() {
                    ...
                    let animation = match preset.animation_style {
                        ...
                        AnimationStyle::SlideUp => format!("{{\\move({x},{},{x},{curr_y},0,180)}}", curr_y + 25),
                        _ => String::new(),
                    };
                    ...
                    if preset.animation_style == AnimationStyle::Karaoke {
                        for (idx, token) in visual.split_whitespace().enumerate() {
                            ...
                            let centiseconds = words.get(global_word)
                                .map(|w| ((w.end - w.start) * 100.0).round().max(1.0) as u64)
                                .unwrap_or(10);
                            rendered.push_str(&format!("{{\\k{centiseconds}}}{}", safe_text(token, preset.uppercase)));
                            global_word += 1;
                        }
                    } else {
                    ...
                    out.push_str(&format!("Dialogue: 0,{},{},Default,{name},0,0,0,,{}\n", format_ass_time(line.start), format_ass_time(line.end), rendered));
                }
            }
        }
    }
...
#[allow(dead_code)]
fn _graphemes_for_future_width(text: &str) -> usize { text.graphemes(true).count() }
...
mod tests {
    ...
    fn sample_line() -> SubtitleLine {
        SubtitleLine { id: 4, start: 1.0, end: 2.0, text: "Bonjour monde".into(), words: None }
    }
