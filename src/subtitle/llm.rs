...
pub async fn correct_lines(
    ...
    let input = lines
        .iter()
        .enumerate()
        .map(|(i, line)| format!("{marker}{i}:{}", line.text.replace('\n', " ")))
        .collect::<Vec<_>>()
        .join("\n");
    let body = json!({
    ...
    let response =
        tokio::select! { r = request.send() => r, _ = token.cancelled() => return lines };
    ...
    for row in content.lines() {
        let Some(rest) = row.trim().strip_prefix(marker) else {
            continue;
        };
        ...
        if let Ok(index) = index.parse::<usize>()
            && index < corrected.len()
        {
            corrected[index] = Some(text.trim().to_owned());
        }
    }
    if corrected.iter().any(Option::is_none) {
        return lines;
    }
    lines
        .into_iter()
