...
fn srt_time(seconds: f64) -> String {
    ...
    let h = total / 3_600_000; let m = (total / 60_000) % 60; let s = (total / 1000) % 60; let ms = total % 1000;
...
pub fn parse_srt_to_lines(text: &str) -> Vec<SubtitleLine> {
    ...
    for block in normalized.split("\n\n") {
        ...
        let timing = if first.contains("-->") { first } else { rows.next().unwrap_or_default() };
        let Some((start, end)) = timing.split_once("-->") else { continue; };
        let (Some(start), Some(end)) = (parse_time(start), parse_time(end.split_whitespace().next().unwrap_or_default())) else { continue; };
        let body = rows.collect::<Vec<_>>().join("\n").trim().to_string();
        if body.is_empty() { continue; }
        lines.push(SubtitleLine { id: lines.len() as u32, start, end, text: body, words: None });
    }
...
pub fn generate_srt_content(lines: &[SubtitleLine]) -> String {
    ...
    for (idx, line) in lines.iter().enumerate() {
        out.push_str(&format!("{}\n{} --> {}\n{}\n\n", idx + 1, srt_time(line.start), srt_time(line.end), line.text));
    }
    out
}
...
fn ass_time(value: &str) -> Option<f64> {
    ...
    let h: f64 = parts.next()?.parse().ok()?; let m: f64 = parts.next()?.parse().ok()?; let s: f64 = parts.next()?.parse().ok()?;
    Some(h * 3600.0 + m * 60.0 + s)
}

fn strip_ass_tags(text: &str) -> String {
    let mut out = String::with_capacity(text.len()); let mut in_tag = false;
...
pub fn parse_ass_to_lines(text: &str) -> Vec<SubtitleLine> {
    ...
    for raw in text.lines().filter_map(|line| line.strip_prefix("Dialogue:")) {
        let parts: Vec<&str> = raw.splitn(10, ',').collect();
        if parts.len() < 10 { continue; }
        let (Some(start), Some(end)) = (ass_time(parts[1]), ass_time(parts[2])) else { continue; };
        ...
        if let Some(existing) = out.iter_mut().find(|line| line.text == text && (line.start - start).abs() < 0.08 && end <= line.end + 0.25) {
            existing.start = existing.start.min(start); existing.end = existing.end.max(end); continue;
        }
        out.push(SubtitleLine { id: out.len() as u32, start, end, text, words: None });
    }
    normalize_subtitles(&out, NormalizeOptions::default()).lines
}

pub fn lines_from_words(words: Vec<SubtitleWord>) -> Vec<SubtitleLine> {
    words.into_iter().enumerate().map(|(id, word)| SubtitleLine { id: id as u32, start: word.start, end: word.end, text: word.word.clone(), words: Some(vec![word]) }).collect()
}
...
mod tests {
    ...
    fn srt_round_trip_preserves_text() {
        ...
        let lines = parse_srt_to_lines(input); assert_eq!(lines[0].text, "Bonjour\nmonde");
