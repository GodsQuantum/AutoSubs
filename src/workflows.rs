...
pub async fn run_supervisor(state: AppState, shutdown: CancellationToken) {
    ...
    for token in state.watcher_tokens.iter() {
        token.value().1.cancel();
    }
}
...
async fn watch_workflow(
    ...
    let mut watcher: Option<RecommendedWatcher> =
        ...
        .ok();
    let native_error = watcher
        .as_mut()
        .and_then(|w| w.watch(&watch_dir, RecursiveMode::NonRecursive).err());
    if let Some(error) = native_error {
        tracing::warn!(%error, "native watcher unavailable; reconciliation scan remains active");
        watcher = None;
    }
    let mut interval = tokio::time::interval(Duration::from_secs(
        state.config.workflow_scan_seconds.max(1),
    ));
...
async fn try_process_path(
    ...
    if token.is_cancelled() || !path.is_file() || is_sidecar_or_staging(path) {
        return;
    }
    ...
    if state
        .db
        .workflow_seen(&workflow.id, &path_text, size, mtime)
        .unwrap_or(false)
    {
        attempted.insert(path.to_path_buf(), (size, mtime));
        return;
    }
    if !crate::media::probe::is_video(path, token).await {
        attempted.insert(path.to_path_buf(), (size, mtime));
        return;
    }
    attempted.insert(path.to_path_buf(), (size, mtime));
    match process_workflow_file(state, workflow, path, token).await {
        Ok(()) => {
            if let Err(error) = state
                .db
                .mark_workflow_seen(&workflow.id, &path_text, size, mtime)
            {
                tracing::warn!(%error, "could not persist workflow dedupe fingerprint");
            }
        }
        Err(error) => {
            tracing::error!(workflow = %workflow.name, path = %path.display(), %error, "workflow processing failed; source was not archived by the watcher")
        }
    }
}
...
async fn process_workflow_file(
    ...
    let job = jobs::create_job(
        ...
        path.to_path_buf(),
        sidecar,
        workflow.preset_id.clone(),
        Some(workflow),
    )?;
    jobs::enqueue_prepare(state.clone(), job.id.clone())?;
    wait_for(&state.clone(), &job.id, supervisor, |status| {
        matches!(
            status,
    ...
    if prepared.status != JobStatus::Ready {
        return Err(anyhow!(
            prepared
                .error
                .unwrap_or_else(|| format!("job ended as {:?}", prepared.status))
        ));
    }
    ...
    if rendered.status == JobStatus::Done {
        Ok(())
    } else {
        Err(anyhow!(rendered.error.unwrap_or_else(|| format!(
            "job ended as {:?}",
            rendered.status
        ))))
    }
...
async fn stable_fingerprint(
    path: &Path,
    delay: Duration,
    token: &CancellationToken,
) -> Result<(u64, i128)> {
    let a = tokio::fs::metadata(path).await?;
    if !a.is_file() {
...
fn is_sidecar_or_staging(path: &Path) -> bool {
    let name = path
        ...
        .unwrap_or_default()
        .to_ascii_lowercase();
    if name.contains(".partial-") || name.ends_with(".uploading") || name.ends_with("_words.json") {
        return true;
    }
    matches!(
        path.extension()
...
mod tests {
    ...
    fn staging_and_sidecars_are_not_video_candidates() {
        for name in [
            ...
            assert!(is_sidecar_or_staging(Path::new(name)), "{name}");
        }
